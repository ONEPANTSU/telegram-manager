...
class AuthMethods:
    ...
    async def start(
            ...
            *,
            ...
            last_name: str = '',
            max_attempts: int = 3, state) -> 'TelegramClient':
        """
        ...
        if self.me is not None:
            ...
            if bot_token:
                ...
                if bot_token[:bot_token.find(':')] != str(self.me.id):
                    warnings.warn(
                        'the session already had an authorized user so it did '
                        'not login to the bot account using the provided '
                        'bot_token (it may not be using the user you expect)'
                    )
            elif phone and not callable(phone) and utils.parse_phone(phone) != self.me.phone:
                warnings.warn(
                    'the session already had an authorized user so it did '
    ...
    async def _start(
        ...
        while attempts < max_attempts:
            try:
                ...
                self.me = await self.sign_in(phone, code=value)
                break
